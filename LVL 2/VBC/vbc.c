...
{
    node *ret = calloc(1, sizeof(*ret));  // MODIFIED: sizeof(*ret) instead of sizeof(n)
    if (!ret)
        return NULL;
    *ret = n;
    return ret;
...
/* MODIFIED: accept/expect restored to original signature, without global */
...
/* ADDED: parsing a factor (digit or parenthesis) */
static node *parse_factor(char **s)
{
    ...
    if (accept(s, '(')) {
        ...
        if (!e)
            return NULL;
        if (!expect(s, ')')) {          // MOD: check for closing parenthesis
            destroy_tree(e);
            return NULL;
        }
        return e;
...
/* MODIFIED: parse_expr initializes a local pointer and checks end of string */
